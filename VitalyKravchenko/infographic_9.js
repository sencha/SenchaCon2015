...
Ext.define('Infographic.View', {
    ...
    title: 'Infographic: Step 9 (creating a custom sprite to place text along a curve and using it in the chart)',
    floating: true,
    constrain: true,
...
Ext.define('Infographic.ArcText', { // TODO 1
...
Ext.define('Infographic.Controller', {
    ...
    onAfterRender: function (chart) {
        ...
        chart.on({
            ...
            single: true // only do this on first 'layout' event
        });
    },
    ...
    onHighlightSliceRender: function (sprite, config, data, index) {
        ...
        if (label !== '' && label !== 'year' && me.regionIndex !== ln) {
            ...
            if (label === region.start) { // found the start of a region
                ...
                // Label region lines with text sprites. // TODO 2
                region.sprites.push(overlaySurface.add({
                    type: 'arctext',
                    text: region.name,
